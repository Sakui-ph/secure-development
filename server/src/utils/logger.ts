...
const { combine, timestamp, json, colorize, printf, label } =
  format;
...
export const LogError = (error: Error, generic = "An error has occurred", type : LogType = LogType.NONE) => {
...
export const LogWarning = (warning: Error, generic = "A warning has occurred", type : LogType = LogType.NONE) => {
  let time = new Date().toISOString();
  if (process.env.STATUS !== "dev") {
